...
<body>
	<div id="main">
			...
			<tbody>
				<c:forEach items="${list}" var="sco">
					<tr data-id="${sco.id} ">
						...
						<td class="value"><input data-id="${sco.id}" data-empid="${sco.emp.id}" data-proid="${sco.pro.id}" name="value" value="${sco.value}"></td>
		...
		<ul class="pagination">
			<li id="pre"><a
				href="SS?type=manage&ye=${p.ye-1}&name=${c.emp.name}&depName=${c.emp.dep.name}&proName=${c.pro.name}&value=${c.value!=-1?c.value:''}">上一页</a></li>
			<c:forEach begin="${p.beginYe}" end="${p.endYe}" varStatus="status">
				<li <c:if test="${p.ye==status.index}">class="active"</c:if>><a
					href="SS?type=manage&ye=${status.index}&name=${c.emp.name}&depName=${c.emp.dep.name}&proName=${c.pro.name}&value=${c.value!=-1?c.value:''}">${status.index}</a></li>
			</c:forEach>
			<li id="next"><a
				href="SS?type=manage&ye=${p.ye+1}&name=${c.emp.name}&depName=${c.emp.dep.name}&proName=${c.pro.name}&value=${c.value!=-1?c.value:''}">下一页</a></li>
		</ul>
